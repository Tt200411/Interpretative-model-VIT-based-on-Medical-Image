...
# Assuming NORM_EPS is defined somewhere in your code
NORM_EPS = 1e-5
...
class PatchEmbed(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # 确保卷积层的定义可以处理不同的输入通道数
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False)

        self.norm = norm_layer(out_channels)
        self.avgpool = nn.Identity() if stride == 1 else nn.AvgPool2d((2, 2), stride=2, ceil_mode=True,
                                                                      count_include_pad=False)

    def forward(self, x):
        x = self.avgpool(x)  # 先应用池化
        x = self.conv(x)  # 然后应用卷积
        x = self.norm(x)  # 最后应用归一化
        return x
...
class E_MHSA(nn.Module):
    ...
    def merge_bn(self, pre_bn):
        merge_bn(self.q, pre_bn)
        if self.sr_ratio > 1:
            merge_bn(self.k, pre_bn, self.norm)
            merge_bn(self.v, pre_bn, self.norm)
        else:
            merge_bn(self.k, pre_bn)
            merge_bn(self.v, pre_bn)
        self.is_bn_merged = True
...
class MedViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        self.stem = nn.Sequential(
            ...
            ConvBNReLU(stem_chs[2], 64, kernel_size=3, stride=2),
        )
